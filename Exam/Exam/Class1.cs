...
namespace Exam
{
    public class Kritic
    {
        ...
        public Kritic(string path)
        {
            ...
            using (StreamWriter sr = new StreamWriter("Vivod.txt"))
            {
        ...
        struct Rbt
        {
            public int point1;// точка начала
            public int point2;// точка конца
            public int length;// путь
        ...
        int Mv(List<Rbt> ls, Rbt minel)//записывают точки начало путей
        {
            int ret = 0;
        ...
        int Mv(List<Rbt> ls, Rbt minel, StreamWriter sr)// ищем поиск путей
        {
            int ret = 0;
            Rbt rb = ls.Find(x => x.point1 == minel.point1 && x.point2 == minel.point2);
            sr.WriteLine(rb.point1 + " - " + rb.point2);
